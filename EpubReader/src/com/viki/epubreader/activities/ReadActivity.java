...
public class ReadActivity extends ReaderBaseActivity {
	...
	private void openChapter(int chapNo) {
		...
		height = height
				- Math.round(pageGap * Math.round(mDisplayMetrics.density));
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case (MotionEvent.ACTION_UP):
					...
					if (clickDuration < MAX_CLICK_DURATION
							&& Math.abs(posXFirst - posX) <= epsilonClick) {
						NextOrPrevPage(posX);
					}
					// else if(clickDuration>=MAX_LONG_CLICK_DURATION &&
					// Math.abs(posXFirst-posX)<=epsilonClick){
	...
	/**
	 * chia deu 4 phan bang nhau. Neu o phan thu 1 va phan thu 4 thi chuye?n
	 * page con lai danh cho menu(phan thu 2 va thu 3)
	 * */
	protected void NextOrPrevPage(float posX) {
		...
		if ((posX >= 0 && posX <= widthClick)
				|| (posX >= mDisplayMetrics.widthPixels - widthClick && posX <= mDisplayMetrics.widthPixels)) {
			// prev click
			if (posX >= 0 && posX <= widthClick) {
				if (mState.curPage >= 1) {
					mBrowser.loadUrl(String.format(
							"javascript:setPageNumber(%d)", mState.curPage - 1));
				}
			}
			// next click
			else if (posX >= mDisplayMetrics.widthPixels - widthClick
					&& posX <= mDisplayMetrics.widthPixels) {
				if (mState.curPage < mState.totalPagesOfChapter) {
					mBrowser.loadUrl(String.format(
							"javascript:setPageNumber(%d)", mState.curPage + 1));
				}
			}
	...
	public String getCurrentFontSize() {
		String fontSize = ""; /*
							 * = pref.getString(
							 * SettingActivity.KEY_FONT_SIZE_PREFERENCE,
							 * "medium");
							 */
		int nFontSize = 20;
